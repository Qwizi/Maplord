...
def _get_available_keys():
    ...
    from apps.game_config.models import BuildingType, UnitType, AbilityType
...
class GameAssetAdmin(ModelAdmin):
    ...
    def preview(self, obj):
        if obj.file and obj.file.name.lower().endswith(('.webp', '.png', '.jpg', '.jpeg', '.gif', '.svg')):
            return format_html('<img src="{}" style="max-height:40px;max-width:60px;object-fit:contain;" />', obj.file.url)
        return obj.file.name.split('/')[-1] if obj.file else '-'
    preview.short_description = 'Preview'

    def preview_large(self, obj):
        if obj.file and obj.file.name.lower().endswith(('.webp', '.png', '.jpg', '.jpeg', '.gif', '.svg')):
            return format_html('<img src="{}" style="max-height:200px;max-width:300px;object-fit:contain;" />', obj.file.url)
    ...
    def available_keys(self, obj=None):
        ...
        for category, key, name, exists in keys:
            ...
            html += format_html(
                ...
                color, status, key, category, name,
            )
        html += '</div>'
