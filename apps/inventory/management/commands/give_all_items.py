...
from apps.inventory.views import add_item_to_inventory, create_item_instance
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        items = Item.objects.filter(is_active=True).select_related('category').order_by(
            'category__order', 'rarity', 'name'
        )
        ...
        for item in items:
            if item.is_stackable:
                ...
                if already_owns:
                    instance_skipped += 1
                    self.stdout.write(
                        self.style.WARNING(f'  [INSTANCE SKIP] {item.name} (already owned)')
                    )
                else:
