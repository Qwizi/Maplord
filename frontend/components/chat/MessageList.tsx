...
export function MessageList({ messages, currentUserId }: MessageListProps) {
  ...
  return (
    <div ref={containerRef} className="flex-1 overflow-y-auto px-3 py-2 space-y-1 scrollbar-thin scrollbar-thumb-border">
      {messages.map((msg, i) => {
        ...
        return (
          <div key={`${msg.timestamp}-${i}`} className={`text-xs md:text-sm leading-relaxed ${sameUser ? "" : "pt-1.5"}`}>
            {!sameUser && (
